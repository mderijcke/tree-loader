...
TreeLoader.prototype.start = function() {
	...
	this.watcher.on("ready", () => {
		...
		var tasks = this.queue.map((f) => this.loadFile(f[0], f[1]));
	...
	watcher.on("add", (f, stat) => this.loadFile(f, stat));
	watcher.on("change", (f, stat) => this.loadFile(f, stat));
	watcher.on("unlink", (f, stat) => this.removeFile(f, stat));
};
...
TreeLoader.prototype.loadFile = Promise.coroutine(function *(fileName, contents) {
	if (!this.ready) {
...
TreeLoader.prototype.removeFile = function(fileName) {
	if (!this.ready) {
		var index = this.queue.indexOf(fileName);
